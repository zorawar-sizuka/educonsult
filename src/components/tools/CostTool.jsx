...
//     try {
//       // First log the run to get an ID
...
//         setSaveMsg("Save failed — could not log run");
//         setSaving(false);
//         return;
//       }
...
//       // Then copy to ToolSaved
//       const sessionId = getSessionId();
//       const savedRes = await fetch("/api/tools/saved", {
//         method: "POST",
...
//       setSaveMsg(savedJson.ok ? "Saved! Check History → Saved." : "Save failed");
//     } catch {
...
export default function CostTool({ countryOptions = [], countryMap = new Map(), restore }) {
  ...
  // Detect if we have real DB data
  const hasDbData = countryOptions.length > 0;
  ...
  // Prefer DB if we have DB data
  const livingMonthlyUsd = hasDbData
    ? (selected?.livingCostMonthlyUsd ?? 1200)
    : (livingCosts?.[inputs.country] ?? 1200);

  const tutionMonthlyUsd = hasDbData
    ? (selected?.tutionCostMonthlyUsd ?? 1200)
    : (tutionCosts?.[inputs.country] ?? 1200);

  // Costs calculations
  const livingYearUsd = livingMonthlyUsd * 12;
  const tuitionYearUsd = tutionMonthlyUsd * 2; // placeholder logic (kept)
  const totalYearUsd = livingYearUsd + tuitionYearUsd;
