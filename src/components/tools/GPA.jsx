...
import { 
  ...
  CheckCircle2, FileText, ChevronRight, School, Minus, Plus 
} from 'lucide-react';
...
// Exact client-side fallback (your original logic)
const clientCalculate = (stream, mode, marks = {}, subjectGpas = {}) => {
  ...
  if (mode === 'gpaToMarks') {
    ...
    for (const sub of subjects) {
      ...
      if (val < 0.8 || val > 4.0) return null; // Invalid
...
export default function GPACalculator() {
  ...
  const handleGpaChange = (id, val) => {
    if (val === '' || (parseFloat(val) >= 0.8 && parseFloat(val) <= 4.0)) {
  ...
  const calculate = async () => {
    ...
    const payload = {
      stream,
      mode,
      marks: mode === 'marksToGpa' ? marks : undefined,
      subjectGpas: mode === 'gpaToMarks' ? subjectGpas : undefined
    };

    try {
      const res = await fetch("/api/tools/gpa", {
        method: "POST",
      ...
      console.warn("[GPA] Backend failed—using client fallback", e);
    }
  ...
  const downloadPDF = () => {
    ...
    autoTable(doc, {
      ...
      styles: { fontSize: 10, cellPadding: 6, lineColor: [226, 232, 240], textColor: [51, 65, 85] },
    });
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-900 p-4 md:p-8">
      <div className="max-w-5xl mx-auto mb-10 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <button
          onClick={() => { setMode(prev => prev === 'marksToGpa' ? 'gpaToMarks' : 'marksToGpa'); setResult(null); setMarks({}); setSubjectGpas({}); }}
          className="flex items-center gap-2 px-5 py-2.5 rounded-lg text-sm font-bold text-slate-900 bg-[#bbf250] hover:brightness-105 transition-all shadow-sm"
        >
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 space-y-6">
          <div className="flex gap-2 flex-wrap">
            {Object.keys(STREAMS).map(key => (
              <button
                key={key}
                onClick={() => { setStream(key); setResult(null); setMarks({}); setSubjectGpas({}); }}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                  ...
                  {mode === 'marksToGpa' ? (
                    <div className="flex gap-3 w-full">
                      <div className="relative w-full">
                        <input 
                          type="number" 
                          placeholder={sub.th.toString()} 
                          value={marks[`${sub.id}_th`] || ''}
                      ...
                      <div className="relative w-full">
                        <input 
                          type="number" 
                          placeholder={sub.pr.toString()} 
                          value={marks[`${sub.id}_pr`] || ''}
                          className="w-full bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-slate-900 focus:outline-none transition-all font-mono"
            ...
            <div className="p-6 pt-0">
              <PillButton onClick={calculate} className="w-full py-4" disabled={loading}>
        ...
        <div className="lg:col-span-5 space-y-6">
            ...
            {result ? (
              ...
              >
                ...
                <div className="p-8">
                  ...
                  <button 
                    onClick={downloadPDF}
                    className="w-full py-4 rounded-xl font-bold text-sm text-emerald-950 bg-emerald-300/80 hover:bg-emerald-300 transition-colors flex items-center justify-center gap-2 shadow-sm mb-4"
                  >
                  ...
                  {saveMsg && <p className="mt-3 text-sm text-center text-emerald-700">{saveMsg}</p>}
                </div>
              </motion.div>
            ) : (
          ...
          {/* Grading Reference Table (unchanged) */}
