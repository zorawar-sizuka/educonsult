...
// import React, { useMemo, useState } from 'react';
// import { AnimatePresence, motion } from 'framer-motion';
// import { Clock, RotateCcw, Star, X, Trash2 } from 'lucide-react';
...
//     activeTab === "calculator" ? "cost" :
//     activeTab === "sop" ? "sop" :
//     activeTab === "discovery" ? "finder" :  // ← matches DB value "finder"
//     activeTab === "gpa" ? "gpa" :
//     ""
//   ), [activeTab]);

//   const [view, setView] = useState("all"); // "all" = recent, "saved" = starred/pinned

//   // Recent runs (max 10, pruned automatically)
...
//   // Saved runs (separate table, max 25)
//   const { runs: savedRuns, loading: savedLoading, error: savedError, refresh: refreshSaved } = useSavedToolHistory({
//     toolType: toolType || "",
//     take: 25,
//   });

//   const runs = view === "all" ? recentRuns : savedRuns;
//   const loading = view === "all" ? recentLoading : savedLoading;
//   const error = view === "all" ? recentError : savedError;
//   const refresh = () => {
//     refreshRecent();
//     refreshSaved();
//   };
...
//     // Determine if this run is already saved
//     const isCurrentlySaved = savedRuns.some(s => 
//       s.originalRunId === run.id || s.id === run.id
//     );
//     const action = isCurrentlySaved ? "unsave" : "save";
//     const runId = run.originalRunId || run.id; // works for both tables
...
//                 <div className="text-xs font-bold uppercase text-slate-400">History</div>
//                 <div className="text-lg font-bold text-slate-900">
//                   {view === "saved" ? "Saved 25" : "Recent 10"} Tool Runs
...
//                 <div className="p-6 rounded-xl bg-slate-50 border border-slate-100 text-center text-slate-500">
//                   {view === "saved" 
//                     ? "No saved runs yet. Star a run to pin it here." 
...
//               {!loading && !error && runs.map((r) => {
//                 const isSaved = savedRuns.some(s => 
//                   s.originalRunId === r.id || s.id === r.id
//                 );
...
//                           )}
//                           {r.toolType === "gpa" && (
//                             <>Mode: {r.payload?.mode || "Unknown"} • GPA: {r.result?.gpa ?? r.payload?.gpaToMarks ?? "-"}</>
//                           )}
//                           {r.toolType === "finder" && (
//                             <>University Finder • Filters applied</>
//                           )}
//                         </div>
